...
def sb_stech_windows(input_file, size_name, p):
    import pandas as pd
    import pathlib
    import os
...
#size_name = "10k"
    ...
    #df_p = df.query('p-value < p')
    df_p = df[df['p-value'] < float(p)]
    df_ps = df_p.sort_values(['chr','position_start'])
    df_ps.to_csv(stech_tmp_file, index=False, header=True, sep='\t')
    ...
    with open(stech_tmp_file, 'r') as win, open(stech_file, 'w') as sout:
        ...
        for line in win:
            ...
            if chrom == chrom1 and int(start1)<int(start) and int(start)<int(end1):
                ...
                rec = str(chrom1)+'\t'+str(start0)+'\t'+str(end1)+'\t'+str(count_set)+'\t'+str(p_set)+'\t'+str(p_min)+'\t'+str(sample_set)+'\n'
                sout.write(rec)
                chrom1 = chrom
                start0 = start
                start1 = start
...
if __name__== "__main__":
    ...
    parser.add_argument("--p",action="store", metavar = "p-value threshold", default = '0.001', type = float,
                            help = "p-value threshold [0.001]") 
    ...
    sb_stech_windows(input_file, size_name, p)
    
"""    
size=10000
size_name='10k'
p=0.001
input_file='merge_col_BBNX_break.cs.ann.fil.strict.txt'
"""